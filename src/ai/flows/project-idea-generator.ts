...
const prompt = ai.definePrompt({
  ...
  **User's Choices:**
  ...
  {{#if frontend}}- **Frontend:** {{{frontend}}}{{/if}}
  {{#if backend}}- **Backend:** {{{backend}}}{{/if}}
  {{#if ai_ml}}- **AI/ML:** {{{ai_ml}}}{{/if}}
  {{#if databases}}- **Databases:** {{{databases}}}{{/if}}
  {{#if devops}}- **DevOps:** {{{devops}}}{{/if}}
  ...
  **Instructions:**
  ...
  3.  **Define the Project Roadmap in a Tree Structure:** Output the roadmap with the following four main steps. Each step must have a title, an array of detailed sub-steps, and an array of high-quality resource links (name and URL) to official documentation, key libraries, or critical tutorials.
      ...
      - **Step 2: Project setup:** Detail the installation process for the core technologies, suggest a file structure, and include links to official documentation for the main frameworks/libraries.
      - **Step 3: Section-wise breakdown:** Explain the logical order of features to build. For example: 1) User Authentication, 2) Core Feature A, 3) Core Feature B, etc.
      - **Step 4: Deployment & publishing:** Describe unit/integration testing steps and suggest deployment options from the user's list or recommend suitable ones (e.g., Vercel, Netlify for frontend; Heroku, Render for backend).
