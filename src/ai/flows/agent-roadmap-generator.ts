...
const prompt = ai.definePrompt({
  ...
  **Instructions:**
  ...
  3.  **JSON Output:** Create a valid n8n workflow JSON string that a user can directly import.
      ...
      - **Node 3 (aiNode):** A "Google Gemini" node. Set the "Model" to "gemini-1.5-flash-latest" and the "Text" field to an expression that references the prompt from the Set node (e.g., {{{{raw}}}}`={{ $('Set Input').item.json.prompt }}`{{{{/raw}}}}).
